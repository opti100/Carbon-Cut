  ...
  switch (event.type) {
    case 'checkout.session.completed':
      ...
      try {
        ...
        console.log(`Processing payment completion for report: ${reportId}, email: ${userEmail}`);
        ...
        if (report && userEmail) {
          try {
            ...
            if (emailSent) {
              console.log(`✅ Certified PDF sent successfully to ${userEmail} for report ${reportId}`);
            } else {
          ...
          console.error('Report or email not found:', { reportExists: !!report, userEmail });
        }
