...
const Page = () => {
  ...
  const [onboardingType, setOnboardingType] = useState<'ads' | 'website'>(
    typeParam || 'ads'
  )
  ...
  const adsSteps = [
    {
      ...
      content: (handleContinue: () => void) => (
        <OnboardingGoogleAds onNext={handleContinue} />
      ),
    },
    {
      id: 2,
      content: (handleContinue: () => void) => (
        <ApiKeyStep onNext={handleContinue} sourceType="ads" />
      ),
    },
    {
      id: 3,
      content: (handleContinue: () => void) => (
        <CampaignCreationStep onComplete={handleContinue} />
      ),
    },
  ]
        ...
        {onboardingType === 'ads' ? (
          <div className="space-y-2">
            ...
            <p className="text-gray-600 text-sm max-w-md">
              Connect your Google Ads account, install our SDK, and start tracking carbon
              emissions from your campaigns
            </p>
          </div>
        ) : (
          <div className="space-y-2">
