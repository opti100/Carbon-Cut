...
export default class GHGProtocolCarbonCalculator {
  ...
  async getGridIntensity(
    country: string
  ): Promise<{ intensity: number; source: 'IEA_API' | 'FALLBACK'; updated: string }> {
    try {
      ...
      console.log(
        `✅ Grid intensity for ${country}: ${result.averageIntensity} gCO2/kWh (${result.dataSource}, updated: ${result.lastUpdated})`
      )
      ...
      const fallbackIntensity =
        this.emissionFactors.grid_intensity[country] ||
        this.emissionFactors.grid_intensity['default']
      console.log(
        `Using fallback grid intensity for ${country}: ${fallbackIntensity} gCO2/kWh`
      )
  ...
  calculateAdProduction(
    travelKm: number,
    travelMode: string,
    powerKWh: number,
    materials: { type: string; kg: number }[],
    cloudGBMonths: number,
    gridIntensity: number
  ): CalculationResult {
  ...
  calculateDigitalMedia(
    ...
    if (includeDeviceEnergy && viewTimeMinutes > 0) {
      deviceEmissions =
        I * viewTimeMinutes * this.emissionFactors.device_energy_mobile * gridFactor
    }
  ...
  calculateOwnedSocialPosts(
    ...
    if (includeDeviceEnergy && views > 0 && viewTimeMinutes > 0) {
      deviceEmissions =
        views * viewTimeMinutes * this.emissionFactors.device_energy_mobile * gridFactor
    }
  ...
  calculateEmailMarketing(
    ...
    if (includeDeviceEnergy && opens > 0 && readTimeMinutes > 0) {
      deviceEmissions =
        opens * readTimeMinutes * this.emissionFactors.device_energy_mobile * gridFactor
    }
    ...
    return {
      ...
      methodology:
        'Email Marketing: ESP energy + Data transfer + Optional reading energy',
    }
  }
  ...
  calculateWebsite(
    ...
    const dataTransferEmissions =
      visits * dataMB * this.emissionFactors.network_data_transfer * gridFactor
  ...
  calculatePrint(
    ...
    const paperEF =
      paperType === 'recycled'
        ? this.emissionFactors.paper_recycled
        : this.emissionFactors.paper_virgin
  ...
  calculateOOH(
    type: 'static' | 'digital',
    materialsKg: number,
    installTripsKm: number,
    disposalKg: number,
    screenPowerKW: number,
    hours: number,
    dataGB: number,
    gridIntensity: number
  ): CalculationResult {
    ...
    if (type === 'static') {
      ...
      const installEmissions =
        installTripsKm * this.emissionFactors.ooh_installation_vehicle
      ...
      const dataEmissions =
        dataGB * 1000 * this.emissionFactors.network_data_transfer * gridFactor
  ...
  calculateEvents(
    venueKWh: number,
    boothMaterialsKg: number,
    attendeeTravelKm: number,
    travelMode: string,
    freightTonKm: number,
    freightMode: string,
    ...
    // Booth materials
    const boothEmissions =
      boothMaterialsKg * this.emissionFactors.booth_materials_standard
    ...
    return {
      ...
      methodology:
        'Events: Venue energy + Booth materials + Attendee travel + Freight + Catering',
    }
  }

  // 10) Influencer & Creator Marketing
  // CO₂e_creator = Production_footprint + Travel + Cloud/Editing + Content_delivery (use §2–5 formulas)
  calculateInfluencerMarketing(
    productionHours: number,
    productionType: string,
    travelKm: number,
    travelMode: string,
    cloudGBMonths: number,
    contentDeliveryData: { impressions: number; dataGB: number },
    gridIntensity: number
  ): CalculationResult {
    ...
    const deliveryResult = this.calculateDigitalMedia(
      contentDeliveryData.impressions,
      contentDeliveryData.dataGB,
      0,
      gridIntensity
    )
    ...
    return {
      total:
        productionEmissions + travelEmissions + cloudEmissions + deliveryResult.total,
      breakdown: {
      ...
      methodology:
        'Influencer Marketing: Production + Travel + Cloud/Editing + Content delivery',
    }
  }
  ...
  calculatePRPress(
    printContribution: number,
    parcelFreightTonKm: number,
    eventsContribution: number,
    ...
    return {
      total:
        printContribution + freightEmissions + eventsContribution + travelContribution,
      breakdown: {
  ...
  calculateOfficeEnergyAllocation(
    officeKWh: number,
    ...
    return {
      ...
      methodology:
        'Office Energy Allocation: Office energy × Grid factor × Allocation share',
    }
  }
  ...
  private getProductionEmissionFactor(type: string): number {
    ...
    return (
      productionMap[type.toLowerCase()] || this.emissionFactors.video_production_energy
    )
  }
  ...
  async calculateWithGHGProtocol(userInput: GHGUserInput): Promise<GHGProtocolResult> {
    try {
      ...
      const {
        ...
        updated: gridIntensityUpdated,
      } = await this.getGridIntensity(country)
      ...
      switch (activityType.toLowerCase()) {
        ...
        case 'impressions':
          ...
          calculation = this.calculateDigitalMedia(
            quantity,
            dataGB,
            userInput.viewTimeMinutes || 0.5,
            gridIntensity,
            includeDeviceEnergy
          )
          break
        ...
        case 'email':
          ...
          calculation = this.calculateEmailMarketing(
            quantity,
            emailDataGB,
            opens,
            1,
        ...
        case 'social_engagement':
          ...
          calculation = this.calculateOwnedSocialPosts(
            quantity,
            views,
            0.5,
            gridIntensity,
        ...
        case 'web_visit':
          ...
          const bytesPerVisit = userInput.dataGB
            ? userInput.dataGB * 1024 * 1024 * 1024
            : this.emissionFactors.bytes_per_visit_default
          const hostingKWh = userInput.powerKWh || quantity * 0.000001
          calculation = this.calculateWebsite(
            quantity,
            bytesPerVisit,
            hostingKWh,
            gridIntensity
        ...
        case 'paper_kg':
          // Print Materials
          const printingKWh =
            quantity * this.emissionFactors.printing_energy_per_page * 500 // Estimate pages per kg
          const freightTonKm = quantity * 0.001 // Estimate freight
          calculation = this.calculatePrint(
            quantity,
            'virgin',
            printingKWh,
            freightTonKm,
        ...
        case 'production_hour':
          ...
          calculation = this.calculateAdProduction(
            prodTravelKm,
            'car_petrol',
            prodPowerKWh,
            prodMaterials,
        ...
        case 'material_merch_kg':
          ...
          calculation = this.calculateMerchandise(
            items,
            merchFreightTonKm,
            'truck_local',
            merchPackagingKg,
        ...
        case 'content_piece':
          ...
          const infDeliveryData = {
            impressions: quantity * 1000,
            dataGB: quantity * 0.01,
          }
          calculation = this.calculateInfluencerMarketing(
            infProdHours,
            'video',
            infTravelKm,
            'car_petrol',
        ...
        case 'longhaul_air_km':
          // Team Travel
          const travelMode = activityType
            .replace('_km', '')
            .replace('shorthaul_air', 'flight_shorthaul')
            .replace('longhaul_air', 'flight_longhaul')
        ...
        case 'office_energy_kwh':
          ...
          calculation = this.calculateOfficeEnergyAllocation(
            quantity,
            allocationShare,
            gridIntensity
          )
      ...
      const finalBreakdown = {
        digital_processing:
          calculation.breakdown.ad_tech ||
          calculation.breakdown.platform_energy ||
          calculation.breakdown.creation ||
          0,
        energy_consumption:
          calculation.breakdown.data_transfer ||
          calculation.breakdown.power ||
          calculation.breakdown.screen_power ||
          calculation.breakdown.cdn_origin ||
          calculation.breakdown.hosting ||
          calculation.breakdown.printing_energy ||
          calculation.breakdown.venue_energy ||
          calculation.breakdown.cloud_services ||
          calculation.breakdown.allocated_office_energy ||
          0,
        device_energy: calculation.breakdown.device_energy || 0,
        supply_chain:
          calculation.breakdown.freight || calculation.breakdown.parcel_freight || 0,
        materials:
          calculation.breakdown.paper ||
          calculation.breakdown.materials ||
          calculation.breakdown.items ||
          calculation.breakdown.booth_materials ||
          calculation.breakdown.packaging ||
          0,
        transport:
          calculation.breakdown.travel ||
          calculation.breakdown.installation ||
          calculation.breakdown.attendee_travel ||
          0,
        waste: calculation.breakdown.disposal || 0,
        catering: calculation.breakdown.catering || 0,
        cloud_storage:
          calculation.breakdown.cloud || calculation.breakdown.cloud_editing || 0,
        production: calculation.breakdown.production || 0,
        other:
          calculation.breakdown.print ||
          calculation.breakdown.events ||
          calculation.breakdown.content_delivery ||
          0,
      }

      const totalFromBreakdown = Object.values(finalBreakdown).reduce(
        (sum, value) => sum + value,
        0
      )
      ...
      const result: GHGProtocolResult = {
        ...
        data: {
          ...
          recommendations: this.generateGHGRecommendations(
            totalFromBreakdown,
            activityType,
            channel
          ),
  ...
  generateGHGRecommendations(
    ...
    if (totalEmissions > 0.01)
      recommendations.push(
        'Consider renewable energy sources to reduce grid intensity impact.'
      )

    if (
      activityType.match(/impression|digital/i) ||
      channel.match(/Digital Ads|Search & Social Ads/i)
    ) {
    ...
    if (activityType.match(/email/i) || channel.match(/Email/i)) {
      recommendations.push(
        'Optimize email list to reduce sending to inactive recipients.'
      )
