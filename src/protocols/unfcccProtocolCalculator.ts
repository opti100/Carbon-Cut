...
export default class UNFCCCProtocolCarbonCalculator {
  ...
  constructor() {
    ...
    this.ipccFactors = {
      ...
      user_device_consumption_desktop: 0.04, // kgCO2e per hour
      ...
      transport_air_freight_avg: 0.55, // kgCO2e per tonne-km
  ...
  getNationalGridIntensity(country: string): number {
    const intensity =
      this.ipccFactors.national_grid_intensity[country] ||
      this.ipccFactors.national_grid_intensity['default']
    console.log(
      `UNFCCC: Grid intensity for ${country} is ${intensity} gCO2e/kWh (Reflects 2025 NDC progress)`
    )
    return intensity
  }
  ...
  calculateDigitalActivity(
    ...
    const transmissionEmissions =
      dataGB * this.ipccFactors.data_transmission_global_average
  ...
  calculatePhysicalActivity(
    ...
    const logisticsEmissions =
      transportTonneKm * this.ipccFactors.transport_road_freight_avg
    ...
    try {
      ...
      switch (activityType.toLowerCase()) {
        ...
        case 'web_visit':
          ...
          calculation = this.calculateDigitalActivity(
            dataGB,
            processingGB,
            deviceHours,
            gridIntensity,
      ...
      const result: UNFCCC_ProtocolResult = {
        ...
        data: {
          ...
          breakdown: {
            ...
            scope3_supply_chain_materials:
              calculation.breakdown.scope3_supply_chain_materials || 0,
            scope3_supply_chain_logistics:
              calculation.breakdown.scope3_supply_chain_logistics || 0,
          },
