...
export const useAnalytics = () => {
  ...
  const trackUserAction = useCallback(
    (action: string, category: string = 'user_interaction') => {
      if (typeof window !== 'undefined' && window.CarbonCutAnalytics) {
  ...
  const trackCarbonCalculation = useCallback(
    (data: { campaignType?: string; emissions?: number; cost?: number }) => {
  ...
  const trackOffsetPurchase = useCallback(
    (data: { amount: number; projectType: string; cost: number }) => {
  ...
  const trackCertificationRequest = useCallback(
    (data: { reportType: string; cost?: number }) => {
