...
export class GridIntensityService {
  ...
  private static cache = new Map<
    string,
    { data: GridIntensityResult; timestamp: number }
  >()
  ...
  static async getGridIntensity(country: string): Promise<GridIntensityResult> {
    try {
      ...
      if (cached && Date.now() - cached.timestamp < this.CACHE_DURATION) {
        console.log(
          `Using cached grid intensity for ${country}: ${cached.data.averageIntensity} gCO2/kWh`
        )
        return cached.data
      }
  ...
  private static async fetchFromIEA(
    country: string,
    iso3: string
    ...
    const currentHourIntensity = currentHourData
      ? Math.round(currentHourData.Value)
      : averageIntensity

    console.log(
      `✅ IEA API data for ${country} (${iso3}): Average ${averageIntensity} gCO2/kWh, Current hour ${currentHourIntensity} gCO2/kWh`
    )
  ...
  private static getFallbackIntensity(country: string): GridIntensityResult {
    const intensity =
      this.FALLBACK_INTENSITIES[country] || this.FALLBACK_INTENSITIES['default']
