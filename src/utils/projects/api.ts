...
export const fetchProjectsByCountry = async (country: string): Promise<ProjectDTO[]> => {
  try {
    const response = await api.get<ProjectDTO[]>(
      `${API_BASE_URL}/registry/projects/?country=${country}`
    )
...
export const fetchProjectsByRegion = async (region: string): Promise<ProjectDTO[]> => {
  try {
    const response = await api.get<ProjectDTO[]>(
      `${API_BASE_URL}/registry/projects/?region=${region}`
    )
    return response.data
  } catch (error) {
...
export const fetchProjectsByRegistry = async (
  registry: string
): Promise<ProjectDTO[]> => {
  try {
    const response = await api.get<ProjectDTO[]>(
      `${API_BASE_URL}/registry/projects/?registry=${registry}`
    )
    return response.data
  } catch (error) {
...
export const searchProjects = async (query: string): Promise<ProjectDTO[]> => {
  try {
    const response = await api.get<ProjectDTO[]>(
      `${API_BASE_URL}/registry/projects/?search=${query}`
    )
    return response.data
  } catch (error) {
