...
export default function OnboardingGoogleAds({ onNext }: OnboardingGoogleAdsProps) {
  const { status, isLoading: isAuthLoading, accounts, accountsLoading, switchAccount } = useGoogleAds()
  ...
  return (
    <div className="flex flex-col items-start gap-4 w-full max-w-xs">
      ...
      <Button
        onClick={() => {
          if (!isConnected) {
            window.open("https://ads.google.com", "_blank") // Example redirect for connection
          }
        }}
      ...
      >
        ...
        {isConnected ? "Connected" : "Connect with Google Ads"}
      </Button>
