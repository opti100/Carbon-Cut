...
export default function CampaignCreationStep({ onComplete }: CampaignCreationStepProps) {
  ...
  const toggleUTMSelection = (key: string) => {
    setAutoGeneratedUTMs((prev) => {
      const updated = prev.map((utm) =>
        utm.key === key ? { ...utm, selected: !utm.selected } : utm
      )
  ...
  return (
    <form onSubmit={handleSubmit}>
          ...
          {isLoading ? (
            <Select disabled>
              <SelectTrigger className="w-full">
                <SelectValue
                  placeholder={
                    isSwitchingAccount ? 'Switching accounts...' : 'Loading campaigns...'
                  }
                />
              </SelectTrigger>
            </Select>
      ...
      <div className="flex flex-1 gap-2">
        ...
        <Button
          className="bg-white text-gray-900 hover:bg-gray-100 shadow"
          onClick={() => {
