...
export default function ReportActions({
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setOtpStep('verified')
        setOtpMessage(
          'Email verified successfully! You can now download your PDF report.'
        )
      } else {
  ...
  const handlePDFDownload = async () => {
    ...
    if (
      !pdfFormData.name ||
      !pdfFormData.email ||
      !pdfFormData.companyName ||
      !pdfFormData.phoneNumber
    ) {
    ...
    try {
      ...
      const reportData = {
        ...
        displayCO2Data: activities.reduce(
          (acc, activity) => {
      ...
      if (wantsCertification) {
        ...
        try {
          ...
          if (checkoutResult.success && checkoutResult.url) {
            ...
            alert(
              `Your report has been generated! You will now be redirected to complete the certification payment of $${CERTIFICATION_PRICE}. After payment, the certified PDF will be emailed to you.`
            )
      ...
      if (result.pdfUrl) {
        ...
        const message =
          'PDF report generated and saved to your account! A copy has been sent to your email.'
        alert(message)
  ...
  const downloadReport = async (reportId: string) => {
    try {
      ...
      alert(
        `Error downloading report: ${error instanceof Error ? error.message : 'Please try again.'}`
      )
      return false
    }
  }
      ...
      <CardContent className="p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="text-center md:text-left mb-4 md:mb-0">
          <h4 className="font-bold text-gray-900 text-base md:text-lg">
            Want to neutralise your impact?
          </h4>
          <p className="text-sm text-gray-600 mt-1">
            Talk to us about insetting/offsetting options and how to reduce future
            emissions.
          </p>
        </div>
        <div className="flex flex-wrap gap-2 justify-center md:justify-start w-full md:w-auto">
          ...
          >
            <DialogTrigger asChild>
              <Button className="px-4 sm:px-6 md:px-8 text-white text-sm sm:text-base bg-[#6c5f31] hover:bg-[#b0ea1d] ">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <DialogDescription className="text-gray-600 text-left text-sm sm:text-base mt-1">
                        Please provide your details to generate a customized carbon
                        footprints report.
                      </DialogDescription>
                    </div>
                  </div>
                      ...
                      <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                            ...
                            uploadProgress.includes('saved') ? (
                              <Cloud className="h-4 w-4 text-blue-600" />
                            ) : uploadProgress.includes('database') ||
                              uploadProgress.includes('activities') ? (
          ...
          <Link href="/offset">
            <Button className="bg-[#b0ea1d] text-white font-medium text-sm px-4 py-2.5 h-auto rounded-lg shadow-sm hover:shadow-md transition-all duration-200 w-full sm:w-auto">
