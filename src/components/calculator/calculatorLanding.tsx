...
export default function CalculatorLanding() {
  ...
  const { calculatingEmissions, emissionResults, calcCO2AI, getDisplayCO2 } =
  ...
  const handleAddActivity = () => {
    ...
    const newActivity: Omit<ActivityData, 'id'> = {
      date: campaignPeriod?.from
        ? format(campaignPeriod.from, 'yyyy-MM-dd')
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      ...
      const hasQuantities = Object.values(activityQuantities).some(
        (q) => parseFloat(q) > 0
      )
      if (!hasQuantities) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#fcfdf6' }}>
      ...
      >
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6 lg:gap-8 min-h-0">
          <div
            className="lg:col-span-2 rounded-xl p-3 sm:p-4 md:p-6 lg:p-8 flex flex-col h-auto min-h-[450px] sm:min-h-[500px] md:h-[500px] lg:h-[600px]"
          ...
          >
            ...
            <div
              className="flex justify-between items-center pt-4 sm:pt-6 shrink-0 gap-2 sm:gap-4"
              style={{ borderTop: '1px solid #d1cebb' }}
            >
              {currentStep > 1 ? (
                <Button
                  onClick={handleBack}
      ...
      {activities.length > 0 && (
        <>
          <div
            ...
            className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 min-h-screen"
