...
import {
  ...
  loadCountriesFromScientificData,
} from '@/constants/data'
...
export default function MarketingCalculator({
  ...
  const [calculatingEmissions, setCalculatingEmissions] = useState<
      ...
      if (activity.quantities && Object.keys(activity.quantities).length > 0) {
        ...
        for (const [unitKey, data] of Object.entries(activity.quantities)) {
          ...
          try {
            ...
            console.log(
              `✅ API Result for ${unitKey} (${data.label}):`,
              emissions,
              'kg CO₂e'
            )
            ...
            setEmissionResults((prev) => ({
              ...prev,
              [individualActivityId]: fallbackEmission,
            }))
            totalEmissions += fallbackEmission
          } finally {
            setCalculatingEmissions((prev) => ({
              ...prev,
              [individualActivityId]: false,
            }))
          }
        }
        ...
        console.log(
          `✅ Total emissions for activity #${activityId}:`,
          totalEmissions,
          'kg CO₂e'
        )
    ...
    (activity: ActivityData): number => {
      ...
      if (activity.quantities && Object.keys(activity.quantities).length > 0) {
        ...
        return Object.entries(activity.quantities).reduce((sum, [unitKey, data]) => {
          return (
            sum +
            FALLBACK_CALCULATION.getBasicEmission({
              ...activity,
              unit: unitKey,
              qty: data.value,
            })
  ...
  const updateActivity = useCallback(
    (activityId: number, updates: Partial<ActivityData>) => {
      setActivities((prev) =>
        prev.map((activity) => {
  ...
  const calculateTotals = useCallback(() => {
    ...
    activities.forEach((activity) => {
      ...
      byScope[activity.scope as keyof typeof byScope] =
        (byScope[activity.scope as keyof typeof byScope] || 0) + co2
    })
